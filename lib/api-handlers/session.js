...
function insuredStatusLabel(status, uploadCount) {
  if (status === "broker_managed") {
    return "Broker record — add email to invite uploads";
  }
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (wantLaunch) {
      ...
      var launchBase = String(iftaDeploymentUrl || "")
        .trim()
        .replace(/\/+$/, "");
      if (/\/reports$/i.test(launchBase)) {
        launchBase =
          launchPayload.insuredId != null && typeof launchPayload.insuredId === "number"
            ? launchBase + "/insured-summary"
            : launchBase + "/upload";
      }
      var launchUrl = appendLaunchToken(launchBase, tok);
        ...
        try {
          ...
          payload.insureds = list.map(function (r) {
            return {
              ...
              lastReportSource: r.lastReportSource || "",
            };
          });
        } catch (ie) {
