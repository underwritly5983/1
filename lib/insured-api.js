...
module.exports = async function handleInsuredPost(req, res) {
  try {
    ...
    if (action === "create") {
      var sessC = getBrokerSession(req);
      if (!sessC.ok) {
        return sendJson(res, 401, { ok: false, error: sessC.error });
      }
      var broker = await userStore.getUser(sessC.email);
      if (!broker || !broker.passwordHash) {
        return sendJson(res, 401, { ok: false, error: "Account not found." });
      }
      var name = typeof body.name === "string" ? body.name.trim() : "";
      var em = normEmail(body.email);
      ...
      var created;
      try {
        created = await insuredDb.createInsured(sessC.email, ck, name, em, phone, hash, expires);
      } catch (e) {
        if (e && e.code === "23505") {
    ...
    if (action === "delete_insured") {
      var sessDel = getBrokerSession(req);
      if (!sessDel.ok) {
        return sendJson(res, 401, { ok: false, error: sessDel.error });
      }
      var brokerDel = await userStore.getUser(sessDel.email);
      if (!brokerDel || !brokerDel.passwordHash) {
        return sendJson(res, 401, { ok: false, error: "Account not found." });
      }
      ...
      var delResult = await insuredDb.deleteByBrokerAndId(sessDel.email, delId);
      if (!delResult.ok) {
    ...
    if (action === "resend_insured_email") {
      var sessRe = getBrokerSession(req);
      if (!sessRe.ok) {
        return sendJson(res, 401, { ok: false, error: sessRe.error });
      }
      var brokerRe = await userStore.getUser(sessRe.email);
      if (!brokerRe || !brokerRe.passwordHash) {
        return sendJson(res, 401, { ok: false, error: "Account not found." });
      }
      ...
      var rowRe = await insuredDb.getByBrokerAndId(sessRe.email, reId);
      if (!rowRe) {
      ...
      if (rowRe.status === "completed") {
        return sendJson(res, 400, {
          ok: false,
          error:
            "This insured has already uploaded reports. Remove them only by deleting the insured if needed.",
        });
      }
      if (!insuredMfa.isConfigured() || !insuredToken.getSecret()) {
        return sendJson(res, 503, {
      ...
      if (rowRe.status === "pending_mfa") {
        ...
        var okMfaUp = await insuredDb.updateMfaForPendingInsured(sessRe.email, reId, hashRe, expiresRe);
        if (!okMfaUp) {
    ...
    if (action === "upload") {
      ...
      if (!rowU || rowU.status !== "awaiting_upload") {
        return sendJson(res, 400, {
          ok: false,
          error: "Upload is not allowed for this record (wrong status or already completed).",
        });
      }
      ...
      iftaIngestWebhook.postIftaWebhook(webhookPayload).catch(function () {});
