...
function ensureInsuredSchema() {
  ...
  _schemaPromise = (async function () {
    ...
    `;
    await sql`
      ALTER TABLE insureds ADD COLUMN IF NOT EXISTS last_report_source VARCHAR(32)
    `;
    await sql`
      ALTER TABLE insureds ADD COLUMN IF NOT EXISTS broker_upload_file_count INTEGER NOT NULL DEFAULT 0
    `;
    try {
      await sql`ALTER TABLE insureds ALTER COLUMN email DROP NOT NULL`;
    } catch (eAl) {
      if (eAl && String(eAl.message || "").indexOf("does not exist") < 0) {
        /* ignore if already nullable */
      }
    }
    try {
      await sql`UPDATE insureds SET email = NULL WHERE email = ''`;
    } catch (eUp) {
      /* ignore */
    }
    try {
      await sql`DROP INDEX IF EXISTS insureds_broker_email_unique`;
    } catch (eDr) {
      /* ignore */
    }
    await sql`
...
function rowToPublic(row) {
  ...
  return {
    ...
    lastReportSource: row.last_report_source ? String(row.last_report_source).trim() : "",
  };
}
...
async function listByBroker(brokerEmail) {
  ...
  var rows = await sql`
    SELECT id, broker_email, company_key, name, email, phone, status,
           ...
           broker_upload_file_count,
           (COALESCE(jsonb_array_length(uploads), 0) + COALESCE(broker_upload_file_count, 0)) AS upload_count
    FROM insureds
...
async function getById(id) {
  ...
  var rows = await sql`
    SELECT id, broker_email, company_key, name, email, phone, status,
           mfa_code_hash, mfa_expires_at, created_at, verified_at, completed_at, uploads, last_report_source
...
async function createInsured(
  ...
  var creator =
    createdByEmail && String(createdByEmail).trim()
      ...
      : be;
  var ph = String(phone || "").trim().slice(0, 64) || "";
  var useMfa = !!(mfaHash && mfaExpiresIso);
  var status = options.status || (useMfa ? "pending_mfa" : "broker_managed");
  var lastSrc = options.lastReportSource || null;
  var mfaH = useMfa ? mfaHash : null;
  var mfaE = useMfa ? mfaExpiresIso : null;
  var brokerFiles = parseInt(String(options.brokerUploadFileCount != null ? options.brokerUploadFileCount : 0), 10);
  if (isNaN(brokerFiles) || brokerFiles < 0) brokerFiles = 0;
  if (brokerFiles > 32) brokerFiles = 32;
  var rows = await sql`
    INSERT INTO insureds (broker_email, company_key, name, email, phone, status, mfa_code_hash, mfa_expires_at, created_by_email, last_report_source, broker_upload_file_count)
    VALUES (
      ${be},
    ...
    RETURNING id, broker_email, company_key, name, email, phone, status, created_at, verified_at, completed_at, uploads, created_by_email, last_report_source, broker_upload_file_count
  `;
...
async function appendUploadsAndComplete(id, newFiles) {
  ...
  await sql`
    ...
    SET uploads = COALESCE(uploads, '[]'::jsonb) || ${chunk}::jsonb,
        ...
        last_report_source = 'insured_portal'
    WHERE id = ${n}
  `;
  return getById(n);
}
...
async function updateInsuredEmailByBroker(brokerEmail, insuredId, newEmail) {
  await ensureInsuredSchema();
  var sql = dbPostgres.getSql();
  ...
  if (!be) return null;
  var em =
    newEmail == null || (typeof newEmail === "string" && !String(newEmail).trim())
      ? null
      : normEmail(newEmail);
  var rows = await sql`
    UPDATE insureds
    SET email = ${em}
    WHERE id = ${n} AND broker_email = ${be}
    RETURNING id
  `;
  if (!rows || !rows.length) return null;
  return rowToPublic(await getById(n));
}

async function transitionBrokerManagedToPendingMfa(brokerEmail, insuredId, mfaHash, mfaExpiresIso) {
  await ensureInsuredSchema();
  var sql = dbPostgres.getSql();
  var n = parseInt(insuredId, 10);
  if (isNaN(n) || n < 1) return false;
  ...
  var rows = await sql`
    ...
    SET status = 'pending_mfa',
        mfa_code_hash = ${mfaHash},
        mfa_expires_at = ${mfaExpiresIso}::timestamptz
    WHERE id = ${n} AND broker_email = ${be} AND status = 'broker_managed'
    RETURNING id
  `;
  return !!(rows && rows.length);
}

async function touchLastReportSource(brokerEmail, insuredId, source) {
  await ensureInsuredSchema();
  var sql = dbPostgres.getSql();
  var n = parseInt(insuredId, 10);
  if (isNaN(n) || n < 1) return false;
  var be = normEmail(brokerEmail);
  var src = String(source || "").trim().slice(0, 32);
  if (!src) return false;
  await sql`
    UPDATE insureds
    SET last_report_source = ${src}
    WHERE id = ${n} AND broker_email = ${be}
  `;
  return true;
}
...
async function getOrgDashboardStats(brokerEmail) {
  ...
  var agg = await sql`
    SELECT
      ...
      COALESCE(SUM(COALESCE(jsonb_array_length(uploads), 0) + COALESCE(broker_upload_file_count, 0)), 0) AS total_files
    FROM insureds
    WHERE broker_email = ${be}
  `;
  var insuredCount = 0;
...
module.exports = {
  ...
  updateInsuredEmailByBroker: updateInsuredEmailByBroker,
  transitionBrokerManagedToPendingMfa: transitionBrokerManagedToPendingMfa,
  touchLastReportSource: touchLastReportSource,
};
