...
async function createInsured(brokerEmail, companyKey, name, email, phone, mfaHash, mfaExpiresIso) {
  await ensureInsuredSchema();
  var sql = dbPostgres.getSql();
  var be = normEmail(brokerEmail);
  var em = normEmail(email);
  var rows = await sql`
    INSERT INTO insureds (broker_email, company_key, name, email, phone, status, mfa_code_hash, mfa_expires_at)
    VALUES (
      ${be},
    ...
    RETURNING id, broker_email, company_key, name, email, phone, status, created_at, verified_at, completed_at, uploads
  `;
...
async function appendUploadsAndComplete(id, newFiles) {
  ...
  var merged = existing.concat(newFiles || []);
  var now = new Date().toISOString();
  var js = JSON.stringify(merged);
  await sql`
    UPDATE insureds
    SET uploads = ${js}::jsonb,
        status = 'completed',
